...
5. ⬅️ **IN PROGRESS (at step 3): frontend API resync — see [`FRONTEND-RESYNC-PLAN.md`](FRONTEND-RESYNC-PLAN.md) (repo root).** `Frontend/ServiceLink/` is a real Angular 21 app (Tailwind 4, vitest) that this file never tracked — and it was written against the *pre-Phase-1* API, so it had drifted badly. Sequenced **before attachments** because attachments need both halves anyway, and because a working UI is a better contract test than another mocked suite (cf. the `@Query` bugs in §3 that only the context test caught). `npm install` first — `node_modules` starts empty; no Docker needed for frontend work.
   ...
   - ⬜ **Still open — step 3 onward.** Most important: **a ticket's status still can't be changed from the UI** — `updateTicketStatus` exists in the service with *no caller*, so the capability is one component away rather than absent. Also open: the unguarded `getUserById(ticket.assignedTo)` that requests `/api/users/null` for unassigned tickets; status badges styling only 3 of 6 statuses; `dashboard.openTickets` counting only `NEW`; **the bearer token being `console.log`'d in `jwt.interceptor.ts`**; and every `.spec.ts` still a CLI stub with no `provideHttpClient`. The plan doc carries the full inventory plus four findings from the step-2 session (notably: `Ticket.comments` has **no `@OrderBy`**, so the comment order embedded in `TicketResponseDto` is undefined — PR #7's oldest-first guarantee covers only the comment *endpoints*, which the frontend doesn't call).

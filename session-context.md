...
5. ⬅️ **NEXT: frontend API resync — see [`FRONTEND-RESYNC-PLAN.md`](FRONTEND-RESYNC-PLAN.md) (repo root).** `Frontend/ServiceLink/` is a real Angular 21 app (Tailwind 4, vitest) that this file never tracked — and it was written against the *pre-Phase-1* API, so it has drifted badly: `assignTicket` and `updateComment` use the wrong HTTP verb (405), the `Status` enum has 3 of 6 values, `PageResponse` expects the flat Spring shape instead of `VIA_DTO`'s nested `$.page`, and **there is no way to change a ticket's status at all** (it moved to `PATCH /{id}/status` and out of the update DTO). The plan doc has the full verified inventory, step-by-step fixes, and an end-to-end verification script. Sequenced **before attachments** because attachments need both halves anyway, and because a working UI is a better contract test than another mocked suite (cf. the `@Query` bugs in §3 that only the context test caught). `npm install` first — `node_modules` starts empty; no Docker needed for frontend work.

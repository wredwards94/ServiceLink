...
**Attachments.** ✅ *Done.*
File and screenshot uploads on **tickets and comments** (PR #13, `d7c9e07`), stored as Postgres `bytea` on a dedicated `@SoftDelete` `Attachment` entity. `byte[] data` is deliberately **not** `@Lob` — Hibernate 6 + PG maps `@Lob byte[]` to `oid` (large-object), not `bytea`. No `StorageService` seam: with `bytea` the bytes are part of the row, so the abstraction would be premature; moving to filesystem/S3 later is a deliberate refactor swapping `data` for a path/key. Uploads arrive as `multipart/form-data` (`@RequestParam("file") MultipartFile` — a part is not the body, so *not* `@RequestBody`), so there is no request DTO: `AttachmentMapper` is one-directional (entity → `AttachmentResponseDto`) and the inbound direction is hand-written in the service, wrapping `getBytes()`'s checked `IOException` as a `BadRequestException` (clean 400 instead of an unhandled 500). All routes live on one `AttachmentController` (`/api/attachments`) following the existing sub-resource convention — the child owns its controller and reaches up via a path segment (`POST|GET /ticket/{ticketId}`, `POST|GET /comment/{commentId}`), exactly like `CommentController`, so `TicketController` and `CommentController` were not touched; plus `GET /{id}/download` (`ResponseEntity<byte[]>` with `Content-Type` + `Content-Disposition`) and staff-only `DELETE /{id}`. Because every route sits under `/api/attachments/**`, the existing `/api/tickets/**` and `/api/comments/**` rules do **not** cover them — the new `SecurityConfig` rules are required for the endpoints to be reachable at all, not merely a hardening step. Authorization reuses `TicketAccessPolicy.assertCanView` (a USER only touches their own tickets' attachments) and keeps attachments on **internal** comments staff-only, mirroring the comment rules. Validation: 10 MB cap + content-type allowlist + empty-file rejection; `spring.servlet.multipart.*` raised to 10 MB (Boot defaults to 1 MB/file, which would reject uploads before the service check runs) with `MaxUploadSizeExceededException` handled as a 400. List endpoints use constructor-expression JPQL so the `bytea` column is **never** loaded just to return metadata. Covered by Mockito service tests (ownership 403, internal-comment 403, size/type/empty 400) and a `@WebMvcTest` slice using `MockMultipartFile`. Frontend UI landed in PR #14 — see *Frontend* below.

*Trap:* `AttachmentResponseDto.id` must stay lowercase — MapStruct property matching is case-sensitive and its default `unmappedTargetPolicy` is WARN, so a capitalized `Id` compiles with only a warning and silently emits `id = 0`. Read the generated `AttachmentMapperImpl` when a DTO field looks unmapped.

*Deferred:* an aggregated "all attachments on a ticket incl. its comments" view, uploader-initiated delete (staff-only for now), and virus scanning.
...
## Phase 3 — Collaboration, notifications & SLAs (next)
...
- ✅ **Comment endpoint cleanups.** *Done.* Comment reads now default to `createdAt` **ascending** (conversation order — oldest first) on both `GET /api/comments/ticket/{ticketId}` and `.../search`; `TicketController` deliberately stays descending, since newest-first is right for a ticket list. Added the missing leading slash to `@GetMapping("/ticket/{ticketId}/search")` and removed the stale "add searching" / "add pagination" TODO markers describing work that had shipped. Also dropped a dead commented-out `findAllByTicketId(List<Long>)` line from `TicketRepository`. `CommentControllerTest` now captures the `Pageable` via `ArgumentCaptor` and asserts `Sort.Direction.ASC` on `createdAt` (verified to fail if the sort is flipped back). *(Two items on the original list — the orphaned non-paginated `List<Comment> findAllByTicketId(Long)` overload and stale `List` imports in `CommentRepository` — turned out to have been cleaned up already. `TicketRepository`'s `java.util.List` import is live and stays.)* The commented-out logging block in `CommentController` is intentionally kept as the tracker for Phase 4 Observability.
